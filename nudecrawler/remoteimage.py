...
class RemoteImage:
    def __init__(self, url: str, page_url: str = None):
        ...
        try:
            ...
            msg = f"Requests.exception for {self.url}: {e}"
            raise ProblemImage(msg)

        # r.raise_for_status()
        if r.status_code != 200:
            msg_0 = f"Bad status {r.status_code} for {self.url}"
            raise ProblemImage(msg_0)
    ...
    def detect_image(self, script):
        ...
        env = os.environ.copy()
        if self.page_url:
            env["NUDECRAWLER_PAGE_URL"] = self.page_url
        env["NUDECRAWLER_IMAGE_URL"] = self.url
        rc = subprocess.run([script, self.path], env=env)
