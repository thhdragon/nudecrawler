...
def main():
    ...
    for cls in ["hentai", "porn", "sexy"]:
        thresholds[cls] = float(
            os.getenv(f"DETECTOR_THRESHOLD_{cls.upper()}", detector_threshold_str)
        )

    c = detect_image(
        image_path, address=detector_address, thresholds=thresholds, verbose=verbose
    )
    sys.exit(c)
