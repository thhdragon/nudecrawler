...
def get_args(argv: str, methods_list: list[str], context_fields: list):
    ...
    parser = argparse.ArgumentParser(
        description=f"Nudecrawler: Telegra.ph Spider {__version__}\nhttps://github.com/yaroslaff/nudecrawler",
    ...
    parser.add_argument(
        ...
        type=int,
        metavar=("MONTH", "DAY"),
        help="Current date (default is today) example: --day 12 31",
    )
    parser.add_argument(
        "-c",
        "--config",
    ...
    parser.add_argument(
        "--unbuffered",
        "-b",
        default=config["unbuffered"],
        action="store_true",
        help="Use unbuffered stdout",
    )
    ...
    g.add_argument(
        ...
        "--days",
        type=int,
        default=config["depth"]["days"],
        help=f"check N days back ({config['depth']['days']})",
    )
    g.add_argument(
        "-f",
        "--fails",
    ...
    g.add_argument(
        ...
        default=False,
        action="store_true",
        help="do not detect, print all found pages",
    )
    g.add_argument(
        "--expr",
        "-e",
        metavar="EXPR",
        default=config["filter"]["expr"],
    ...
    g.add_argument(
        ...
        metavar="N",
        type=int,
        default=config["filter"]["total"],
        help=f"Skip detections if less then N total images ({config['filter']['total']})",
    )
    g.add_argument(
        "--max-errors",
        metavar="N",
        type=int,
        default=config["filter"]["max-errors"],
        help=f"Max allowed errors on page ({config['filter']['max-errors']})",
    )
    g.add_argument(
        "--min-content-length",
        metavar="N",
        type=int,
        default=config["filter"]["min-content-length"],
        help="Skip page if content-length less then N (try 5000 or higher)",
    )
    ...
    g.add_argument(
        "--detect",
        metavar="METHOD",
        default=config["detect"]["detect"],
        help=f"One of {methods_list} ({config['detect']['detect']})",
    )
    g.add_argument(
        "--extensions",
        nargs="*",
        default=config["detect"]["extensions"],
        help="interesting extensions (with dot, like .jpg)",
    )
    g.add_argument(
        "--minsize",
        type=int,
        default=config["detect"]["minsize"],
        help=f"min size of image in Kb ({config['detect']['minsize']})",
    )
    g.add_argument(
        "--max-pictures",
        type=int,
        ...
        help="Detect only among first prefiltered N pictures",
    )
    ...
    g.add_argument(
        "--cache",
        metavar="PATH",
        default=config["cache"]["cache"],
        help="path to cache file (will create if missing)",
    )
    g.add_argument(
        "--cache-save",
        type=int,
        metavar="N",
        default=config["cache"]["cache-save"],
        help=f"Save cache after N new images ({config['cache']['cache-save']})",
    )
    ...
    g.add_argument(
        "--detect-image",
        metavar="SCRIPT",
        default=config["detector"]["detect-image"],
        help="explicitly use this script to detect nudity on image file",
    )
    g.add_argument(
        "--detect-url",
        metavar="SCRIPT",
        default=config["detector"]["detect-url"],
        help="explicitly use this script to detect nudity on image URL",
    )

    g = parser.add_argument_group("Long-run options")
    g.add_argument(
        "--log",
        default=config["longrun"]["log"],
        help="print all precious treasures to this logfile",
    )
    ...
    g.add_argument(
        "--workdir",
        default=config["longrun"]["workdir"],
        help=f"Use all files (log, wordlist, cache) in this dir. def: {config['longrun']['workdir']}",
    )
    ...
    g.add_argument(
        ...
        help="periodical statistics file",
    )
    g.add_argument(
        "--resume",
        metavar="STATS_FILE",
        help="resume from STATS_FILE (other args are not needed)",
    )
    g.add_argument(
        "--stop",
        type=int,
        default=config["longrun"]["stop"],
        metavar="NUM_IMAGES",
        help="stop (or --refresh) after N images processed (or little after)",
    )
    g.add_argument(
        "--refresh",
        metavar=("SCRIPT", "ARG"),
        nargs="+",
        help="run this refresh script every --stop NUM_IMAGES images",
    )
    g.add_argument(
        "-j",
        "--workers",
...
def get_default_config():
    ...
    def_config = {
        ...
        "cache": {"cache": os.getenv("NUDE_CACHE"), "cache-save": 1000},
        "longrun": {
